# rate_counters.py
# BA 2015-05-28: created
# dumping rate counters and prescales

from tdf.extern import argparse

MAX_ALGORITHMS = 512
LUMI_SEC = 23.312

parser = argparse.ArgumentParser()
parser.add_argument('device')
parser.add_argument('--limit', type=int, default=MAX_ALGORITHMS, help="limit number of displayed algorithms (default {0})".format(MAX_ALGORITHMS))
args = parser.parse_args(TDF_ARGS)

rateCntFinor = read(args.device, "gt_mp7_gtlfdl.rate_cnt_finor")
rateCntL1A = read(args.device, "gt_mp7_gtlfdl.rate_cnt_l1a")
rateCntBeforePrescaler = blockread(args.device, "gt_mp7_gtlfdl.rate_cnt_before_prescaler")
rateCntAfterPrescaler = blockread(args.device, "gt_mp7_gtlfdl.rate_cnt_after_prescaler")
prescaleFactor = blockread(args.device, "gt_mp7_gtlfdl.prescale_factor")
masks = blockread(args.device, "gt_mp7_gtlfdl.masks")

list=['L1_ZeroBias',
'L1_SingleEG2_BptxAND',
'L1_SingleMu7',
'L1_SingleEG15',
'L1_ETT15_BptxAND',
'L1_ETT40',
'L1_ETT60',
'L1_HTT75',
'L1_Mu12_EG10',
'L1_Mu5_EG15',
'L1_DoubleTauJet40er',
'L1_Mu16er_TauJet20er',
'L1_HTT100',
'L1_SingleIsoEG20',
'L1_SingleJet52',
'L1_SingleJet68',
'L1_SingleJet128',
'L1_DoubleIsoTau32er',
'L1_Mu16er_IsoTau32er',
'L1_DoubleMu0',
'L1_EG25er_HTT100',
'L1_DoubleMuOpen',
'L1_SingleIsoEG25er',
'L1_SingleIsoEG25',
'L1_SingleIsoEG30er',
'L1_SingleEG10',
'L1_SingleIsoEG22er',
'L1_DoubleJetC52',
'L1_DoubleJetC84',
'L1_SingleMu14er',
'L1_DoubleMu_10_Open',
'L1_DoubleMu_10_3p5',
'L1_QuadJetC40',
'L1_SingleEG5',
'L1_SingleEG25',
'L1_SingleEG40',
'L1_SingleIsoEG18er',
'L1_SingleIsoEG20er',
'L1_SingleEG20',
'L1_SingleEG30',
'L1_SingleEG35',
'L1_SingleMuOpen',
'L1_SingleMu16',
'L1_SingleMu5',
'L1_SingleMu20er',
'L1_SingleMu12',
'L1_SingleMu20',
'L1_Mu8_HTT50',
'L1_SingleMu25',
'L1_SingleMu30',
'L1_ETM30',
'L1_ETM50',
'L1_ETM70',
'L1_ETM100',
'L1_HTT125',
'L1_HTT150',
'L1_HTT175',
'L1_HTT200',
'L1_Mu20_EG10',
'L1_Mu5_EG20',
'L1_Mu5_IsoEG18',
'L1_Mu6_DoubleEG10',
'L1_SingleJetC32_NotBptxOR',
'L1_ETM40',
'L1_HTT250',
'L1_Mu10er_ETM30',
'L1_Mu10er_ETM50',
'L1_Mu14er_ETM30',
'L1_DoubleMu7_EG7',
'L1_SingleMu16er',
'L1_Mu6_HTT100',
'L1_Mu4_EG18',
'L1_SingleMuOpen_NotBptxOR',
'L1_DoubleMu6_EG6',
'L1_Mu5_DoubleEG5',
'L1_DoubleEG6_HTT150',
'L1_QuadJetC36_TauJet52',
'L1_Mu0er_ETM40',
'L1_Mu16er_IsoTau28er',
'L1_TripleMu0',
'L1_TripleMu_5_5_3',
'L1_TripleEG_14_10_8',
'L1_DoubleEG_15_10',
'L1_DoubleEG_22_10',
'L1_Mu0er_ETM55',
'L1_ETM60',
'L1_DoubleJetC100',
'L1_QuadJetC60',
'L1_SingleJetC20_NotBptxOR',
'L1_DoubleJetC56_ETM60',
'L1_DoubleIsoTau28er',
'L1_DoubleIsoTau36er',
'L1_DoubleIsoTau40er',
'L1_TripleJet_92_76_64_VBF',
'L1_TripleJet_84_68_48_VBF',
'L1_QuadMu0',
'L1_DoubleMu_12_5',
'L1_SingleMuBeamHalo',
'L1_AlwaysTrue',
'L1_SingleMu7woQcut',
'L1_Mu12woQcut_EG10',
'L1_Mu5woQcut_EG15',
'L1_Mu16erwoQcut_TauJet20er',
'L1_Mu16erwoQcut_IsoTau32er',
'L1_DoubleMu0woQcut',
'L1_SingleMu14woQcut_Eta2p1',
'L1_DoubleMu_10_Open_woQcut',
'L1_DoubleMu_10_3p5_woQcut',
'L1_SingleMuOpenwoQcut',
'L1_SingleMu16woQcut',
'L1_SingleMu5woQcut',
'L1_SingleMu20erwoQcut',
'L1_SingleMu12woQcut',
'L1_SingleMu20woQcut',
'L1_Mu8woQcut_HTT50',
'L1_SingleMu25woQcut',
'L1_SingleMu30woQcut',
'L1_Mu20woQcut_EG10',
'L1_Mu5woQcut_EG20',
'L1_Mu5woQcut_IsoEG18',
'L1_Mu6woQcut_DoubleEG10',
'L1_Mu10erwoQcut_ETM30',
'L1_Mu10erwoQcut_ETM50',
'L1_Mu14erwoQcut_ETM30',
'L1_DoubleMu7woQcut_EG7',
'L1_SingleMu16woQcut_Eta2p1',
'L1_Mu6woQcut_HTT100',
'L1_Mu4woQcut_EG18',
'L1_DoubleMu6woQcut_EG6',
'L1_Mu5woQcut_DoubleEG5',
'L1_Mu0erwoQcut_ETM40',
'L1_Mu16erwoQcut_IsoTau28er',
'L1_TripleMu0woQcut',
'L1_TripleMu_5_5_3woQcut',
'L1_QuadMu0woQcut',
'L1_DoubleMu_12_5woQcut',
'L1_Mu0p5_SingleEG5',
'L1_SingleJet92',
'L1_SingleJet176',
'L1_SingleJet200',
'L1_SingleJet36',
'L1_DoubleJetC112',
'L1_DoubleMu0_Eta1p6_WdEta18',
'L1_DoubleMu0_Eta1p6_WdEta18_OS',
'L1_DoubleMu_10_0_WdEta18',
'L1_IsoEG20er_TauJet20er_NotWdEta0',
'L1_Jet32_DoubleMu_Open_10_MuMuNotWdPhi23_JetMuWdPhi1',
'L1_Jet32_MuOpen_EG10_MuEGNotWdPhi3_JetMuWdPhi1',
'L1_Mu3_JetC16_WdEtaPhi2',
'L1_Mu3_JetC52_WdEtaPhi2',
'L1_SingleEG2',
'L1_SingleEG3',
'L1_SingleJet8',
'L1_SingleJet12',
'L1_SingleJet16',
'L1_ETM20',
'L1_ETT20',
'L1_ETT30',
'L1_HTT30',
'L1_HTT50',
'L1_SingleMu3woQcut',
'L1_BptxAND',
'L1_BptxPlus',
'L1_BptxMinus',
'L1_BptxOR',
'L1_ExtCond_032',
'L1_ExtCond_033',
'L1_ExtCond_034',
'L1_ExtCond_035]


list = list + ['']*(512 - len(list))

print "FinOR rate: {0} Hz".format(int(rateCntFinor / LUMI_SEC))
print "L1A rate: {0} Hz".format(int(rateCntL1A / LUMI_SEC))
print ""
print "| Algo | Rate before prescales (Hz) | Rate after prescales (Hz) | Prescale factor | FINOR mask | Veto | Name |"
for i in range(min(MAX_ALGORITHMS, args.limit)):
    print "| {0:4} | {1:26} | {2:25} | {3:15} | {4:10} | {5:4} | {6} |".format(
        i, int(rateCntBeforePrescaler[i] / LUMI_SEC), int(rateCntAfterPrescaler[i] / LUMI_SEC), prescaleFactor[i], masks[i] & 0x1, masks[i] & 0x2, list[i]
    )

